<template>
  <div class="md:flex md:flex-wrap mt-2 py-4 px-8 mb-4 w-4/5 mx-auto">
      ...
      <label class="block uppercase text-sm text-green-500 mb-2" for="grid-id-crimeType">
         ...
         <div class="relative">
        <select class="block appearance-none w-full bg-white border border-green-500 text-green-500 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-green-500 " id="grid-id-crimeType" v-model="searchFor">
      ...
      <label class="block uppercase text-sm text-green-500 mb-2" for="grid-id-crimeType">
        ...
        <input class="appearance-none block w-full text-green-700 border border-green-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="grid-search" type="address" placeholder="Search" >
      ...
      <label class="block uppercase text-sm text-green-500 mb-2" for="grid-id-crimeType">
        ...
        <button class="bg-green-500 text-white rounded py-2 px-4" >search</button>
      </div>
...
export default {
  ...
  computed:{
    ...
    url(){
      if(this.searchFor == 'status') `/crime?status=${this.selectTerm}`;
      if(this.searchFor == 'type') `/crime?type=${this.selectTerm}`;
      if(this.searchFor == 'BVN') `/allsuspect?BVN=${this.searchTerm}`;
      if(this.searchFor == 'NIN') `/allsuspect?NIN=${this.searchTerm}`;
      if(this.searchFor == 'name') `/allsuspect?name=${this.searchTerm}`;
      if(this.searchFor == 'districtName') `/auth/all?username=${this.searchTerm}`;
    },
    setSelect(){
      let items = [{val: 'Rape', text: 'Rape'},
     ...
     if (this.searchFor = 'status'){
        items =  [
          {val: 'Witness', text: 'Witness'},
          {val: 'Media_Footage_(video/audio)', text: 'Media Footage (video/audio)'},
          {val: 'Red_Handed', text: 'Red Handed'}
        ]
      }
